...
const PremiumPriceStep: React.FC<PremiumPriceStepProps> = ({
  ...
  const handleGradeChange = (grade: GradeValue) => {
    console.log('등급 선택됨:', grade);
    onChange({ productRankCode: grade });
  };

  const handleLocationSelect = (location: string) => {
    console.log('지역 선택됨:', location);
    onChange({ location });
  ...
  useEffect(() => {
    console.log('폼 검증 데이터:', {
      ...
      productRankCode: data.productRankCode,
  ...
  }, [data.productItemCode, data.productVarietyCode, data.productName, data.productRankCode, data.location, data.date, onValidationChange]);
